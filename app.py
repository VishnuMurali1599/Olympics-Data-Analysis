## Here app.py is acting like our webpage
...
if user_menu == "Medal Tally":
    ...
    if selected_years != "Overall" and selected_country != "Overall":
        st.title(selected_country + " perforamance in " + str(selected_years) + " Olympics")
...
if user_menu == 'Country-Wise Analysis':
    ...
    fig_df=helper.country_envent_heatmap(df,selected_country)
    fig,ax = plt.subplots(figsize=(20,20))
    ax = sns.heatmap(fig_df,annot=True)
    st.pyplot(fig)
...
if user_menu == 'Athlete Wise Analysis':
    ...
    fig = px.bar(final, x="Year", y=["Male", "Female"])
    fig.update_layout(autosize=False, width=1000, height=600)
    st.plotly_chart(fig)
