...
def fetch_medal_tally(df,year,country):
    ...
    x['Gold']=x['Gold'].astype('int64')
    x['Silver']=x['Silver'].astype('int64')
    x['Bronze']=x['Bronze'].astype('int64')
    x['total']=x['total'].astype('int64')
    
    return x
    
def medal_tally(df):
    
    medal_tally=medal_tally=df.drop_duplicates(subset=['Team','NOC','Games','Year','City','Sport','Event','Medal'])
    ...
    medal_tally['total']=medal_tally['total'].astype('int64')
...
def country_envent_heatmap(df,country):
    ...
    new_df = temp_df[temp_df['region'] == country]
    fig_df=new_df.pivot_table(index='Sport',columns='Year',values='Medal',aggfunc='count').fillna(0)
